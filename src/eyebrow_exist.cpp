...
EyebrowExist::EyebrowExist()
{
	net = NULL;
	net = new ncnn::Net;
	net->load_param(eyebrow_sim_opt_param_bin);
	net->load_model(eyebrow_sim_opt_bin);
}
...
int EyebrowExist::detect(unsigned char*pInBGRData, int nInCols, int nInRows) {
	...
	ex.input(eyebrow_sim_opt_param_id::BLOB_input, indata);
	ex.extract(eyebrow_sim_opt_param_id::BLOB_output, out);
	if (out.w * out.h * out.c == 2) 
	{ 
		float score = exp(((float*)out.data)[0]) / (exp(((float*)out.data)[0]) + exp(((float*)out.data)[1]));
		std::cout << "eyebrow score : " << score << std::endl;
